...
<?php the_post() ?>
			...
			<div id="post-<?php the_ID() ?>" class="<?php sandbox_post_class() ?>">
				<h3 class="entry-title"><a href="<?php echo get_permalink($post->post_parent) ?>" title="<?php printf( __( 'Return to %s', 'sandbox' ), wp_specialchars( get_the_title($post->post_parent), 1 ) ) ?>" rev="attachment"><?php echo get_the_title($post->post_parent) ?></a> &gt; <?php the_title() ?></h3>
...
<?php edit_post_link( __( 'Edit', 'sandbox' ), "\n\t\t\t\t\t<span class=\"edit-link\">", "</span>" ) ?>
			...
			<div id="nav-images" class="navigation yui-g">
				<div class="nav-previous yui-u first"><?php next_image_link() ?></div>
				<div class="nav-next yui-u"><?php previous_image_link() ?></div>
			</div>
