...
<?php /* the_tags( __( '<p class="tag-links"><span>Tagged:</span> ', 'sandbox' ), ', ', '</p>') */ ?>
				...
				<dl class="entry-meta">
					...
					<dt><?php _e('Author:', 'sandbox')?></dt>
						<dd class="author vcard"><?php printf( __( '%s', 'sandbox' ), '<a class="url fn n" href="' . get_author_link( false, $authordata->ID, $authordata->user_nicename ) . '" title="' . sprintf( __( 'View all posts by %s', 'sandbox' ), $authordata->display_name ) . '"><img src="' . wp_get_attachment_url(492) . '" alt="' . get_the_author() . '" title="' . get_the_author() . '"></a>' ) ?></dd>
<?php						
/*
